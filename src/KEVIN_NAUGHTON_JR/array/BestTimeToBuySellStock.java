...
/*
given 7 nums as stock prices per day of week - calc max proft
cannot sell before buy
buying higer price than selling is LOSS
...
public class BestTimeToBuySellStock {
    ...
    static int maxProfitMultipleTimes(int [] prices){
        ...
        for (int price = 0; price < prices.length-1 ; price++) {
            if(prices[price]<prices[price+1]){//compare next to each other prices
                accumulatedProfit+=prices[price+1]-prices[price];//agregate if next is lower than prev
            }
        }
